...
void BuildInitialTopography( surface *topo, markers *topo_chain, params model, grid mesh, scale scaling ) {
    ...
    for ( k=0; k<topo_chain->Nb_part; k++ ) {
        topo_chain->z[k]     = TopoLevel + h_pert*(3330.0-2800.0)/2800.0*cos(2*PI*topo_chain->x[k]/(xmax-xmin)); //Hplateau = Hroot*(RHOmantle-RHOcrust)/RHOcrust
        topo_chain->phase[k] = 0;
    }
...
void SetParticles( markers *particles, scale scaling, params model, mat_prop *materials  ) {
    ...
    for( np=0; np<particles->Nb_part; np++ ) {
        ...
        if ( particles->z[np] > -HCrust-h_pert*cos(2*PI*particles->x[np]/(xmax-xmin))) particles->phase[np] = 1; // crust above the Moho
     
        
        if ( model.user4 == 1 ) {
            if ( particles->z[np] > -HCrust+2.5e3/scaling.L-h_pert*cos(2*PI*particles->x[np]/(xmax-xmin)) && particles->z[np] < -HCrust+4.5e3/scaling.L-h_pert*cos(2*PI*particles->x[np]/(xmax-xmin))) particles->phase[np] = 7; // marker between 2.5 and 3 km above the Moho
        
            if ( particles->z[np] > -HCrust+7.0e3/scaling.L-h_pert*cos(2*PI*particles->x[np]/(xmax-xmin)) && particles->z[np] < -HCrust+9.0e3/scaling.L-h_pert*cos(2*PI*particles->x[np]/(xmax-xmin))) particles->phase[np] = 8; // marker between 4.5 and 5 km above the Moho
        }
