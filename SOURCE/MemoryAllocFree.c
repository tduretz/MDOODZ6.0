...
void GridAlloc(grid *mesh, params *model) {
  ...
  mesh->FreeSurfW_s =
      (double *)DoodzCalloc(mesh->Nx * mesh->Nz, sizeof(double));
  mesh->FreeSurfW_n =
      (double *)DoodzCalloc((mesh->Nx - 1) * (mesh->Nz - 1), sizeof(double));
  ...
  mesh->rhs_u =
      (double *)DoodzCalloc((mesh->Nx) * (mesh->Nz + 1), sizeof(double));
  ...
  mesh->rhs_v =
      (double *)DoodzCalloc((mesh->Nx + 1) * (mesh->Nz), sizeof(double));
  ...
  mesh->BCu.type =
      (char *)DoodzCalloc((mesh->Nx) * (mesh->Nz + 1), sizeof(char));
  mesh->BCv.type =
      (char *)DoodzCalloc((mesh->Nx + 1) * (mesh->Nz), sizeof(char));
  mesh->BCp.type =
      (char *)DoodzCalloc((mesh->Nx - 1) * (mesh->Nz - 1), sizeof(char));
  mesh->BCp_exp.type =
      (char *)DoodzCalloc((mesh->Nx + 1) * (mesh->Nz + 1), sizeof(char));
  Initialise1DArrayChar(mesh->BCp_exp.type, (mesh->Nx + 1) * (mesh->Nz + 1),
                        -1);
  mesh->BCu.val =
      (double *)DoodzCalloc((mesh->Nx) * (mesh->Nz + 1), sizeof(double));
  mesh->BCv.val =
  ...
  mesh->phase_perc_n =
      (double **)DoodzCalloc(model->Nb_phases, sizeof(double *));
  ...
  if (model->aniso == 1)
    mesh->nx_n = DoodzCalloc((Nx - 1) * (Nz - 1), sizeof(double));
  if (model->aniso == 1)
    mesh->nx_s = DoodzCalloc((Nx - 0) * (Nz - 0), sizeof(double));
  if (model->aniso == 1)
    mesh->nz_n = DoodzCalloc((Nx - 1) * (Nz - 1), sizeof(double));
  if (model->aniso == 1)
    mesh->nz_s = DoodzCalloc((Nx - 0) * (Nz - 0), sizeof(double));
  if (model->aniso == 1)
    mesh->nx0_n = DoodzCalloc((Nx - 1) * (Nz - 1), sizeof(double));
  if (model->aniso == 1)
    mesh->nx0_s = DoodzCalloc((Nx - 0) * (Nz - 0), sizeof(double));
  if (model->aniso == 1)
    mesh->nz0_n = DoodzCalloc((Nx - 1) * (Nz - 1), sizeof(double));
  if (model->aniso == 1)
    mesh->nz0_s = DoodzCalloc((Nx - 0) * (Nz - 0), sizeof(double));
  if (model->aniso == 1)
    mesh->FS_AR_n = DoodzCalloc((Nx - 1) * (Nz - 1), sizeof(double));
  if (model->aniso == 1)
    mesh->FS_AR_s = DoodzCalloc((Nx - 0) * (Nz - 0), sizeof(double));
  if (model->aniso == 1)
    Initialise1DArrayDouble(mesh->FS_AR_n, (Nx - 1) * (Nz - 1), 1.0);
  if (model->aniso == 1)
    Initialise1DArrayDouble(mesh->FS_AR_s, (Nx - 0) * (Nz - 0), 1.0);
  if (model->aniso == 1)
    mesh->aniso_factor_n = DoodzCalloc((Nx - 1) * (Nz - 1), sizeof(double));
  if (model->aniso == 1)
    mesh->aniso_factor_s = DoodzCalloc((Nx - 0) * (Nz - 0), sizeof(double));
...
void GridFree(grid *mesh, params *model) {
  ...
  if (model->aniso == 1)
    DoodzFree(mesh->nz_s);
  if (model->aniso == 1)
    DoodzFree(mesh->nx0_n);
  if (model->aniso == 1)
    DoodzFree(mesh->nx0_s);
  if (model->aniso == 1)
    DoodzFree(mesh->nz0_n);
  if (model->aniso == 1)
    DoodzFree(mesh->nz0_s);
  if (model->aniso == 1)
    DoodzFree(mesh->FS_AR_n);
  if (model->aniso == 1)
    DoodzFree(mesh->FS_AR_s);
  if (model->aniso == 1)
    DoodzFree(mesh->aniso_factor_n);
  if (model->aniso == 1)
    DoodzFree(mesh->aniso_factor_s);
