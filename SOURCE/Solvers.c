...
void kspgcr( cholmod_sparse *M, cholmod_dense *b, cholmod_dense *x, cholmod_factor *Lfact, int N, cholmod_common *c, double eps, int noisy, int *its_tot) {
    ...
    MinMaxArray( f->x, 1, N, "f");
    MinMaxArray( x->x, 1, N, "x");
    MinMaxArray( b->x, 1, N, "b");
...
void kspgcr0( cholmod_sparse *M, cholmod_dense *b, cholmod_dense *x, cholmod_factor *Lfact, int N, cholmod_common *c, double eps, int noisy) {
...
void KillerSolver0( SparseMat *matA,  SparseMat *matB,  SparseMat *matC,  SparseMat *matD, DirectSolver *pardi, double *rhs_mom, double *rhs_cont, double *sol, params model, grid *mesh, scale scaling, SparseMat *Stokes, SparseMat *Jacobian, SparseMat *JmatA,  SparseMat *JmatB,  SparseMat *JmatC ) {
...
//        cholmod_free_dense( &du, &c );
        kspgcr0( Kcm, udum, du, Lfact, matA->neq, &c, model.rel_tol_KSP, noisy);
...
#pragma omp parallel for shared(D1cm0, Dcm0, mesh, Stokes, matA, matD ) private( i ) firstprivate( model, celvol )
    ...
    for ( k=0; k<nitmax; k++) {
        ...
        if ( noisy == 1 ) {
            ...
            printf("PH comp it. %01d. its_KSP = %02d: rel. max. div. = %2.2e -- max. abs. div = %2.2e-- max. rel. div = %2.2e  / max. mom. = %2.2e rel. max. mom. = %2.2e\n", k, its_KSP, fabs(maxdiv/maxdiv0), maxdiv, maxdivit, maxru, maxru/maxru0);
        }
        //        if (fabs(maxdiv/maxdiv0)<rel_tol_div) break;
        //        if (k>0 && fabs(maxdiv)/fabs(maxdivit)>0.75) break;
