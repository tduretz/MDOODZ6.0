...
double Viscosity( int phase, double G, double T, double P, double d, double phi, double exx, double ezz, double exz, double txx0, double tzz0, double txz0, mat_prop* materials, params *model, scale *scaling, int strain_rate_formulation, double *txxn, double *tzzn, double *txzn, double* etaVE, double* VEcoeff, double* Eii_el, double* Eii_pl, double* Eii_pwl, double* Eii_exp , double* Eii_lin, double* Eii_gbs, double* Eii_cst, double* Exx_pwl, double* Exz_pwl, double* Exx_el, double* Ezz_el, double* Exz_el, double* Exx_diss, double* Ezz_diss, double* Exz_diss, double* Exx_pl, double* Exz_pl, double *d1, double* A2_pwl, double strain_acc, double Phi, double C ) {
    ...
    *txxn=0.0; *txzn=0.0; *etaVE=0.0; *VEcoeff=0.0, *A2_pwl=0.0; *Exx_pwl=0.0; *Exz_pwl=0.0, *Exx_el=0.0, *Ezz_el=0.0, *Exz_el=0.0, *Exx_diss=0.0, *Ezz_diss=0.0, *Exz_diss=0.0, *Exx_pl=0.0, *Exz_pl=0.0, *d1=0.0;
...
//    printf("eta_pwl = %2.2e A1pwl = %lf    %2.2e %2.2e %2.2e %2.2e %2.2e %2.2e %2.2e\n", eta_pwl*scaling->eta, A1pwl,    pre_factor, Fpwl , pow(Apwl,-1.0/npwl) , exp( (Eapwl + P*Vapwl)/R/npwl/T ) , pow(d, mpwl/npwl) , pow(fpwl, -rpwl/npwl) , exp(-apwl*phi/npwl));
//    printf("G=%2.2e dt=%2.2e exx=%2.2e ezz=%2.2e exz=%2.2e Exx=%2.2e Exz=%2.2e txx0=%2.2e txz0=%2.2e %d\n", G*scaling->S, dt*scaling->t, exx*scaling->E, ezz*scaling->E, exz*scaling->E, Exx*scaling->E, Exz*scaling->E, txx0*scaling->S, txz0*scaling->S, elastic);
    ...
    if ( local_iter == 0 ) {
        ...
        *VEcoeff                 = 1.0 / (1.0 + G*model->dt/eta);
        ...
        tzz1 = 2.0 * (*etaVE) * Ezz + (*VEcoeff)*tzz0;
        txz1 = 2.0 * (*etaVE) * Exz + (*VEcoeff)*txz0;
        tII1 = sqrt(1.0/2.0*txx1*txx1 + 1.0/2.0*tzz1*tzz1 + txz1*txz1);
...
void NonNewtonianViscosityCells( grid* mesh, mat_prop *materials, params *model, Nparams Nmodel, scale *scaling, int flag ) {
    ...
    double exx_pwl, exz_pwl, A2_pwl, exx_pl, exz_pl;
...
#pragma omp parallel for shared( mesh  ) private( cond, k, l, k1, p, eta, c1, c0, Pn, Tn, txx1, tzz1, txz1, etaVE, VEcoeff, eII_el, eII_pl, eII_pwl, eII_exp, eII_lin, eII_gbs, eII_cst, d1, exx_pwl, exz_pwl, A2_pwl, exx_el, ezz_el, exz_el, exx_diss, ezz_diss, exz_diss, exx_pl, exz_pl ) firstprivate( materials, scaling, flag, average, model, Ncx, Ncz )
    for ( k1=0; k1<Ncx*Ncz; k1++ ) {
        ...
        if ( mesh->BCp.type[c0] != 30 && mesh->BCp.type[c0] != 31) {
            ...
            for ( p=0; p<model->Nb_phases; p++) {
                ...
                if ( cond == 1 ) {
                    eta =  Viscosity( p, mesh->mu_n[c0], Tn, Pn, mesh->d0[c0], mesh->phi[c0], mesh->exxd[c0], mesh->ezzd[c0], mesh->exz_n[c0], mesh->sxxd0[c0], mesh->szzd0[c0], mesh->sxz0_n[c0], materials    , model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1, &A2_pwl, mesh->strain_n[c0], mesh->phi_n[c0], mesh->C_n[c0]);
                }
            ...
            if ( model->cut_noise == 1 ) {
                ...
                mesh->szzd[c0]       = round(mesh->sxxd[c0]*accu)/accu;
                ...
                mesh->ezz_el[c0]    = round(mesh->ezz_el[c0]*accu)/accu;
                mesh->ezz_diss[c0]  = round(mesh->ezz_diss[c0]*accu)/accu;
...
void NonNewtonianViscosityGrid( grid* mesh, mat_prop *materials, params *model, Nparams Nmodel, scale *scaling, int flag ) {
    ...
    int p, k, l, Nx, Nz, Ncx, Ncz, c0, c1, k1, cond;
    double eta, txx1, tzz1, txz1, Pn, Tn, etaVE, VEcoeff=0.0, eII_el, eII_pl, eII_pwl, eII_exp, eII_lin, eII_gbs, eII_cst, d1;
    double exx_pwl, exz_pwl, A2_pwl, exx_el, ezz_el, exz_el, exx_diss, ezz_diss, exz_diss, exx_pl, exz_pl;
    int average = model->eta_avg;
        ...
        for ( iz=0; iz<nd; iz++) {
            ...
            for ( ix=0; ix<nT; ix++) {
                for ( iy=0; iy<nE; iy++) {
                    ...
                    eta =  Viscosity( k, 0.0, T[ix], Pn, d[iz], 0.0, E[iy], E[iy], 0.0, 0.0, 0.0, 0.0, materials, model, scaling, 1, &txx1, &tzz1, &txz1, &etaVE, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl,  &d1, &A2_pwl, 0.0, materials->phi[k], materials->C[k]);
...
void ComputeViscosityDerivatives_FD( grid* mesh, mat_prop *materials, params *model, Nparams Nmodel, scale *scaling, int flag ) {
    ...
    // Evaluate cell center viscosities
#pragma omp parallel for shared( mesh  ) private( eii, cond, k, l, k1, p, eta, c1, c0, Pn, Tn, txx1, tzz1, txz1, etaVE, etaVE_exx, etaVE_ezz, etaVE_exz, etaVE_p, VEcoeff, eII_el, eII_pl, eII_pwl, eII_exp, eII_lin, eII_gbs, eII_cst, d1, exx_pwl, exz_pwl, A2_pwl, exx_el, exz_el, exx_diss, exz_diss, exx_pl, exz_pl, eta_exx, eta_ezz, eta_exz, eta_p, pert_xx, pert_zz, pert_xz , pert_p ) firstprivate( materials, scaling, flag, average, model, Ncx, Ncz, eps, eps1 )
    for ( k1=0; k1<Ncx*Ncz; k1++ ) {
        ...
        if ( mesh->BCp.type[c0] != 30 && mesh->BCp.type[c0] != 31) {
            ...
            for ( p=0; p<model->Nb_phases; p++) {
                ...
                if ( cond == 1 ) {
                    ...
                    eta =  Viscosity( p, mesh->mu_n[c0], Tn, Pn, mesh->d0[c0], mesh->phi[c0], mesh->exxd[c0]+pert_xx, mesh->ezzd[c0], mesh->exz_n[c0], mesh->sxxd0[c0], mesh->szzd0[c0], mesh->sxz0_n[c0], materials    , model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_exx, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1, &A2_pwl, mesh->strain_n[c0], mesh->phi_n[c0], mesh->C_n[c0]);
                    
                    eta =  Viscosity( p, mesh->mu_n[c0], Tn, Pn, mesh->d0[c0], mesh->phi[c0], mesh->exxd[c0], mesh->ezzd[c0]+pert_zz, mesh->exz_n[c0], mesh->sxxd0[c0], mesh->szzd0[c0], mesh->sxz0_n[c0], materials    , model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_ezz, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1, &A2_pwl, mesh->strain_n[c0], mesh->phi_n[c0], mesh->C_n[c0]);
                    
                    eta =  Viscosity( p, mesh->mu_n[c0], Tn, Pn, mesh->d0[c0], mesh->phi[c0], mesh->exxd[c0], mesh->ezzd[c0], mesh->exz_n[c0]+pert_xz, mesh->sxxd0[c0], mesh->szzd0[c0], mesh->sxz0_n[c0], materials    , model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_exz, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1, &A2_pwl, mesh->strain_n[c0], mesh->phi_n[c0], mesh->C_n[c0]);
                    
                    eta =  Viscosity( p, mesh->mu_n[c0], Tn, Pn+pert_p, mesh->d0[c0], mesh->phi[c0], mesh->exxd[c0], mesh->ezzd[c0], mesh->exz_n[c0], mesh->sxxd0[c0], mesh->szzd0[c0], mesh->sxz0_n[c0], materials    , model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_p  , &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1, &A2_pwl, mesh->strain_n[c0], mesh->phi_n[c0], mesh->C_n[c0]);
...
//            }
    ...
    // Calculate vertices viscosity
    double d1s; // dummy variable that stores updated grain size on current vertice
#pragma omp parallel for shared( mesh, model ) private( eii, cond, k, l, k1, p, eta, c1, c0, Ps, Ts, txx1, tzz1, txz1, etaVE, etaVE_exx, etaVE_ezz, etaVE_exz, etaVE_p, VEcoeff, eII_el, eII_pl, eII_pwl, eII_exp, eII_lin, eII_gbs, eII_cst, d1s, exx_pwl, exz_pwl, A2_pwl, exx_el, ezz_el, exz_el, exx_diss, ezz_diss, exz_diss, exx_pl, exz_pl, eta_exx, eta_ezz, eta_exz, eta_p, pert_xx, pert_zz, pert_xz , pert_p ) firstprivate( materials, scaling, flag, average, Nx, Nz, eps, eps1  )
    for ( k1=0; k1<Nx*Nz; k1++ ) {
...
//            pert_p = eps1;
            ...
            for ( p=0; p<model->Nb_phases; p++) {
                ...
                if ( cond == 1 ) {
                    ...
                    eta =  Viscosity( p, mesh->mu_s[c1], mesh->T_s[c1], mesh->P_s[c1], mesh->d0_s[c1], mesh->phi1_s[c1], mesh->exxd_s[c1]+pert_xx, mesh->ezzd_s[c1], mesh->exz[c1], mesh->sxxd0_s[c1], mesh->szzd0_s[c1], mesh->sxz0[c1], materials, model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_exx, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1s, &A2_pwl, mesh->strain_s[c1], mesh->phi_s[c1], mesh->C_s[c1]);
                    
                    eta =  Viscosity( p, mesh->mu_s[c1], mesh->T_s[c1], mesh->P_s[c1], mesh->d0_s[c1], mesh->phi1_s[c1], mesh->exxd_s[c1], mesh->ezzd_s[c1]+pert_zz, mesh->exz[c1], mesh->sxxd0_s[c1], mesh->szzd0_s[c1], mesh->sxz0[c1], materials, model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_ezz, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1s, &A2_pwl, mesh->strain_s[c1], mesh->phi_s[c1], mesh->C_s[c1]);
                    
                    eta =  Viscosity( p, mesh->mu_s[c1], mesh->T_s[c1], mesh->P_s[c1], mesh->d0_s[c1], mesh->phi1_s[c1], mesh->exxd_s[c1], mesh->ezzd_s[c1], mesh->exz[c1]+pert_xz, mesh->sxxd0_s[c1], mesh->szzd0_s[c1], mesh->sxz0[c1], materials, model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_exz, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1s, &A2_pwl, mesh->strain_s[c1], mesh->phi_s[c1], mesh->C_s[c1]);
                    
                    eta =  Viscosity( p, mesh->mu_s[c1], mesh->T_s[c1], mesh->P_s[c1]+pert_p, mesh->d0_s[c1], mesh->phi1_s[c1], mesh->exxd_s[c1], mesh->ezzd_s[c1], mesh->exz[c1], mesh->sxxd0_s[c1], mesh->szzd0_s[c1], mesh->sxz0[c1], materials, model, scaling, flag, &txx1, &tzz1, &txz1, &etaVE_p, &VEcoeff, &eII_el, &eII_pl, &eII_pwl, &eII_exp, &eII_lin, &eII_gbs, &eII_cst, &exx_pwl, &exz_pwl, &exx_el, &ezz_el, &exz_el, &exx_diss, &ezz_diss, &exz_diss, &exx_pl, &exz_pl, &d1s, &A2_pwl, mesh->strain_s[c1], mesh->phi_s[c1], mesh->C_s[c1]);
...
//                // General FD
//                mesh->detadexx_s[c1]      += mesh->phase_perc_s[p][c1] * (etaVE_exx - etaVE) / pert_xx;
//                mesh->detadezz_s[c1]      += mesh->phase_perc_s[p][c1] * (etaVE_ezz - etaVE) / pert_zz;
//                mesh->detadgxz_s[c1]      += mesh->phase_perc_s[p][c1] * (etaVE_exz - etaVE) / pert_xz;
//                mesh->detadp_s[c1]        += mesh->phase_perc_s[p][c1] * (etaVE_p   - etaVE) / pert_p;
...
void ComputeViscosityDerivatives_anal( grid* mesh, mat_prop *materials, params *model, Nparams Nmodel, scale *scaling, int flag ) {
    ...
    double eta_exx, eta_ezz, eta_exz, eta_p;
    double etaVE_exx, etaVE_ezz, etaVE_exz, etaVE_p;
    double eps = 1e-7, pert_xx, pert_zz, pert_xz , pert_p;
    
    double detadexx, detadezz, detadexz;
    double detadexx1, detadezz1, detadexz1;
        ...
        if ( mesh->BCp.type[c0] != 30 && mesh->BCp.type[c0] != 31) {
            ...
            for ( p=0; p<model->Nb_phases; p++) {
                ...
                if ( cond == 1 ) {
                    ...
                    double Exx =mesh->exxd[c0];
                    double Ezz =mesh->ezzd[c0];
                    double Exz =mesh->exz_n[c0];
                    
                    double Eii = sqrt(0.5*pow(Exx,2) + 0.5*pow(Ezz,2) + pow(Exz,2));
                    int    phase = p;
                    double R=materials->R;
                    double d=mesh->d0[c0], phi=mesh->phi[c0], T=mesh->T[c0], P=mesh->p_in[c0];
                    double Eapwl = materials->Qpwl[phase], Vapwl = materials->Vpwl[phase], npwl  = materials->npwl[phase], mpwl  = materials->mpwl[phase], rpwl  = materials->rpwl[phase], Apwl  = materials->Apwl[phase], fpwl = materials->fpwl[phase], apwl = materials->apwl[phase], Fpwl  = materials->Fpwl[phase], pre_factor = materials->pref_pwl[phase];
                    double A1pwl = pre_factor * Fpwl * pow(Apwl,-1.0/npwl) * exp( (Eapwl + P*Vapwl)/R/npwl/T ) * pow(d, mpwl/npwl) * pow(fpwl, -rpwl/npwl) * exp(-apwl*phi/npwl);
                    
                    double  eta_pwl  = A1pwl * pow( Eii, 1.0/npwl - 1.0 );
                    
                    detadexx = 1.0*A1pwl*Exx*(-0.5 + 0.5/npwl)*pow(0.5*pow(Exx, 2) + pow(Exz, 2) + 0.5*pow(Ezz, 2), -0.5 + 0.5/npwl)/(0.5*pow(Exx, 2) + pow(Exz, 2) + 0.5*pow(Ezz, 2));
                    detadezz = 1.0*A1pwl*Ezz*(-0.5 + 0.5/npwl)*pow(0.5*pow(Exx, 2) + pow(Exz, 2) + 0.5*pow(Ezz, 2), -0.5 + 0.5/npwl)/(0.5*pow(Exx, 2) + pow(Exz, 2) + 0.5*pow(Ezz, 2));
                    detadexz = 2*A1pwl*Exz*(-0.5 + 0.5/npwl)*pow(0.5*pow(Exx, 2) + pow(Exz, 2) + 0.5*pow(Ezz, 2), -0.5 + 0.5/npwl)/(0.5*pow(Exx, 2) + pow(Exz, 2) + 0.5*pow(Ezz, 2));
                    
                }
                
                
                if ( cond == 1 ) detadexx1 += mesh->phase_perc_n[p][c0] *detadexx;
                if ( cond == 1 ) detadezz1 += mesh->phase_perc_n[p][c0] *detadezz;
                if ( cond == 1 ) detadexz1 += mesh->phase_perc_n[p][c0] *detadexz;
        ...
        if ( mesh->BCg.type[c1] != 30 ) {
            ...
            for ( p=0; p<model->Nb_phases; p++) {
                ...
                if ( cond == 1 ) {
                    ...
                    double Exx =mesh->exxd_s[c1];
                    double Ezz =mesh->ezzd_s[c1];
                    double Exz =mesh->exz[c1];
                    
                    double Eii = sqrt(0.5*pow(Exx,2) + 0.5*pow(Ezz,2) + pow(Exz,2));
                    int    phase = p;
                    double R=materials->R;
                    double d=mesh->d0_s[c1], phi=mesh->phi1_s[c1], T=mesh->T_s[c1], P=mesh->P_s[c1];
                    double Eapwl = materials->Qpwl[phase], Vapwl = materials->Vpwl[phase], npwl  = materials->npwl[phase], mpwl  = materials->mpwl[phase], rpwl  = materials->rpwl[phase], Apwl  = materials->Apwl[phase], fpwl = materials->fpwl[phase], apwl = materials->apwl[phase], Fpwl  = materials->Fpwl[phase], pre_factor = materials->pref_pwl[phase];
                    double A1pwl = pre_factor * Fpwl * pow(Apwl,-1.0/npwl) * exp( (Eapwl + P*Vapwl)/R/npwl/T ) * pow(d, mpwl/npwl) * pow(fpwl, -rpwl/npwl) * exp(-apwl*phi/npwl);
                ...
                if ( cond == 1 )detadexx1   += mesh->phase_perc_s[p][c1] * detadexx;
                if ( cond == 1 )detadezz1   += mesh->phase_perc_s[p][c1] * detadezz;
                if ( cond == 1 ) detadexz1   += mesh->phase_perc_s[p][c1] * detadexz;
                
            }
            
        }
        
        // Analytical for power law
