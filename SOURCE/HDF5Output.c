...
void WriteOutputHDF5( grid *mesh, markers *particles, surface *topo, markers* topo_chain, params model, char *txtout, mat_prop materials, scale scaling ) {
    ...
    if (model.diffuse_X) P2Mastah( &model, *particles, particles->X,     mesh, mesh->X_n,   mesh->BCp.type,  1, 0, interp, cent    );
    // ---------------------------------------------------------
    // Cast grid arrays
    ...
    P2Mastah( &model, *particles, particles->strain,     mesh, strain,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    P2Mastah( &model, *particles, particles->strain_el,     mesh, strain_el,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    P2Mastah( &model, *particles, particles->strain_pwl,     mesh, strain_pwl,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    P2Mastah( &model, *particles, particles->strain_exp,     mesh, strain_exp,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    P2Mastah( &model, *particles, particles->strain_lin,     mesh, strain_lin,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    P2Mastah( &model, *particles, particles->strain_gbs,     mesh, strain_gbs,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    if ( model.fstrain == 1 ) {
        ...
        P2Mastah( &model, *particles, particles->Fxx,     mesh, Fxx,   mesh->BCp.type,  1, 0, interp, cent    );
        ...
        P2Mastah( &model, *particles, particles->Fxz,     mesh, Fxz,   mesh->BCp.type,  1, 0, interp, cent    );
        ...
        P2Mastah( &model, *particles, particles->Fzx,     mesh, Fzx,   mesh->BCp.type,  1, 0, interp, cent    );
        ...
        P2Mastah( &model, *particles, particles->Fzz,     mesh, Fzz,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    if ( model.aniso == 1 ) {
       ...
       // nx
        ...
        P2Mastah( &model, *particles, particles->nx,     mesh, nx,   mesh->BCp.type,  1, 0, interp, cent    );
        ...
        P2Mastah( &model, *particles, particles->nz,     mesh, nz,   mesh->BCp.type,  1, 0, interp, cent    );
    ...
    if ( model.rec_T_P_x_z == 1 ) {
        ...
        P2Mastah( &model, *particles, particles->T0,     mesh, T0,   mesh->BCp.type,  1, 0, interp, cent    );
        for (k=0; k<(mesh->Nx-1)*(mesh->Nz-1); k++) {
        ...
        P2Mastah( &model, *particles, particles->P0,     mesh, P0,   mesh->BCp.type,  1, 0, interp, cent    );
        for (k=0; k<(mesh->Nx-1)*(mesh->Nz-1); k++) {
        ...
        P2Mastah( &model, *particles, particles->Tmax,     mesh, Tmax,   mesh->BCp.type,  1, 0, interp, cent    );
        for (k=0; k<(mesh->Nx-1)*(mesh->Nz-1); k++) {
        ...
        P2Mastah( &model, *particles, particles->Pmax,     mesh, Pmax,   mesh->BCp.type,  1, 0, interp, cent    );
        for (k=0; k<(mesh->Nx-1)*(mesh->Nz-1); k++) {
        ...
        P2Mastah( &model, *particles, particles->x0,     mesh, x0,   mesh->BCp.type,  1, 0, interp, cent    );
        ...
        P2Mastah( &model, *particles, particles->z0,     mesh, z0,   mesh->BCp.type,  1, 0, interp, cent    );
