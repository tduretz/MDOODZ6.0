...
int main( int nargs, char *args[] ) {
    ...
    if ( irestart == 0 ) {
        ...
        if ( model.no_markers == 0 ) {
            ...
            Interp_P2G ( &particles, materials.eta0,  &mesh, mesh.eta_s, mesh.xg_coord,   mesh.zg_coord, 0, 0, &model, mesh.BCg.type,  Nx,  Nz );
            Interp_P2G ( &particles, materials.eta0,  &mesh, mesh.eta_n, mesh.xc_coord,   mesh.zc_coord, 0, 0, &model, mesh.BCp.type, Ncx, Ncz );
            Interp_P2G ( &particles, particles.T,     &mesh, mesh.T,     mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            Interp_P2G ( &particles, materials.Cv,    &mesh, mesh.Cv,    mesh.xc_coord,   mesh.zc_coord, 0, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            Interp_P2G ( &particles, materials.k_eff, &mesh, mesh.kz,    mesh.xvz_coord,  mesh.zg_coord, 0, 0, &model, mesh.BCv.type, Nx+1, Nz  );
            Interp_P2G ( &particles, materials.k_eff, &mesh, mesh.kx,    mesh.xg_coord,  mesh.zvx_coord, 0, 0, &model, mesh.BCu.type, Nx, Nz+1 );


            if ( model.eqn_state > 0) {
                Interp_P2G ( &particles, particles.rho,  &mesh, mesh.rho_s, mesh.xg_coord,   mesh.zg_coord, 1, 0, &model, mesh.BCg.type,  Nx,  Nz );
                Interp_P2G ( &particles, particles.rho,  &mesh, mesh.rho_n, mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz );
            }
            else {
                Interp_P2G ( &particles, materials.rho,  &mesh, mesh.rho_s, mesh.xg_coord,   mesh.zg_coord, 0, 0, &model, mesh.BCg.type,  Nx,  Nz );
            ...
            Interp_P2G ( &particles, materials.Cv,    &mesh, mesh.Cv,    mesh.xc_coord,   mesh.zc_coord, 0, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            Interp_P2G ( &particles, materials.Qr,    &mesh, mesh.Qr,    mesh.xc_coord,   mesh.zc_coord, 0, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            Interp_P2G ( &particles, particles.T,     &mesh, mesh.T,     mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            Interp_P2G ( &particles, materials.Cv,    &mesh, mesh.Cv,    mesh.xc_coord,   mesh.zc_coord, 0, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            Interp_P2G ( &particles, materials.k_eff, &mesh, mesh.kz,    mesh.xvz_coord,  mesh.zg_coord, 0, 0, &model, mesh.BCv.type, Nx+1, Nz  );
            Interp_P2G ( &particles, materials.k_eff, &mesh, mesh.kx,    mesh.xg_coord,  mesh.zvx_coord, 0, 0, &model, mesh.BCu.type, Nx, Nz+1  );
            ...
            Interp_P2G ( &particles, particles.P,    &mesh, mesh.p_in,    mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            ...
            // Grain size
            Interp_P2G ( &particles, particles.d,    &mesh, mesh.d,    mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz  );
    ...
    for (; model.step<=model.Nt; model.step++) {
        ...
        if (model.no_markers == 0 ) {
            ...
            if ( model.isthermal == 1 ) {
                ...
                Interp_P2G ( &particles, materials.k_eff,    &mesh, mesh.kx,    mesh.xg_coord,  mesh.zvx_coord, 0, 0, &model, mesh.BCu.type, Nx, Nz+1  );
                Interp_P2G ( &particles, particles.T,        &mesh, mesh.T,     mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz  );
                Interp_P2G ( &particles, particles.div_u_th, &mesh, mesh.div_u_th,     mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz  );
            }
...
//                OldDeviatoricStressesPressure( &mesh, &particles, scaling, &model );
                ...
                Interp_P2G ( &particles, particles.rhoUe0,   &mesh, mesh.rhoUe0,  mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz );
            ...
            // Director vector
            if  ( model.aniso == 1 ) {
                Interp_P2G ( &particles, particles.nx,   &mesh, mesh.nx_n,  mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz );
                Interp_P2G ( &particles, particles.nx,   &mesh, mesh.nx_s,  mesh.xg_coord,   mesh.zg_coord, 1, 0, &model, mesh.BCg.type,  Nx,  Nz );
                Interp_P2G ( &particles, particles.nz,   &mesh, mesh.nz_n,  mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz );
                Interp_P2G ( &particles, particles.nz,   &mesh, mesh.nz_s,  mesh.xg_coord,   mesh.zg_coord, 1, 0, &model, mesh.BCg.type,  Nx,  Nz );
            ...
            Interp_P2G ( &particles, particles.phi, &mesh, mesh.phi,  mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz );

            //-----------------------------------------------------------------------------------------------------------
            // Interp P --> p0_n , p0_s
            Interp_P2G ( &particles, particles.P, &mesh, mesh.p0_n,  mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz );
            ...
            Interp_P2G ( &particles, particles.rho, &mesh, mesh.rho0_n,  mesh.xc_coord,   mesh.zc_coord, 1, 0, &model, mesh.BCp.type, Ncx, Ncz );
        }
        else {
