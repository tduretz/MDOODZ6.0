...
muc   = 1;%Adis(h)^(-1/ndis(h))*Ebg^(1/ndis(h)-1)*exp(Qdis(h)/ndis(h)/Rg/Tbg);
...
%     Pt     =    0.* xc2; 
    ...
    while( resnlu/resnlu0 > tol_glob && resnlu>tol_linu/100 && iter < nmax_glob )
        ...
        if lit == 0,
            ...
            [ etac, Txxc, Tyyc, Txyc, detadexxc, detadeyyc, detadexyc ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxc, Eyyc, Exyc, Txxco, Tyyco, Txyco, Bc, mc, etaec );
            [ etav, Txxv, Tyyv, Txyv, detadexxv, detadeyyv, detadexyv ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxv, Eyyv, Exyv, Txxvo, Tyyvo, Txyvo, Bv, mv, etaev );
            ...
            dEyy = toljac*Eiic;
            dExy = toljac*Eiic;
            [ etac_xx, Txxc, Tyyc, Txyc ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxc+dExx, Eyyc     , Exyc     , Txxco, Tyyco, Txyco, Bc, mc, etaec );
            [ etac_yy, Txxc, Tyyc, Txyc ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxc     , Eyyc+dEyy, Exyc     , Txxco, Tyyco, Txyco, Bc, mc, etaec );
            [ etac_xy, Txxc, Tyyc, Txyc ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxc     , Eyyc     , Exyc+dExy, Txxco, Tyyco, Txyco, Bc, mc, etaec );
            ...
            dEyy = toljac*Eiiv;
            dExy = toljac*Eiiv;
            [ etav_xx, Txxv, Tyyv, Txyv ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxv+dExx, Eyyv     , Exyv            , Txxvo, Tyyvo, Txyvo, Bv, mv, etaev );
            [ etav_yy, Txxv, Tyyv, Txyv ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxv     , Eyyv+dEyy, Exyv            , Txxvo, Tyyvo, Txyvo, Bv, mv, etaev );
            [ etav_xy, Txxv, Tyyv, Txyv ] = M2Di2_LocalIteration_VE( litmax, littol, noisy, Exxv     , Eyyv     , Exyv+dExy, Txxvo, Tyyvo, Txyvo, Bv, mv, etaev );
        ...
        if LineSearch==1, [alpha,~] = LineSearch_Direct(BC,nx,ny,dx,dy,u,p,du,dp,Tec,Tev,phc,phv,Rg,Adis,Qdis,ndis,mu_i,mu_0,ksi,0,Newton,0,1/3,noisy,0,resnlu0,resnlp0,resnlu,resnlp,etaec,etaev,Txxco,Tyyco,Txyco,Txxvo,Tyyvo,Txyvo,lit,litmax,littol); end
...
function [alpha, LSsuccess] = LineSearch_Direct(BC,nx,ny,dx,dy,u,p,du,dp,Tec,Tev,phc,phv,Rg,Adis,Qdis,ndis,mu_i,mu_0,ksi,KRO,Newton,comp,pls,noisy,INV,resnlu0,resnlp0,resnlu,resnlp,etaec,etaev,Txxco,Tyyco,Txyco,Txxvo,Tyyvo,Txyvo,lit,litmax,littol)
...
for ils=1:nsteps;
    ...
    if lit == 0,
        ...
        [ etac, Txxc, Tyyc, Txyc ] = M2Di2_LocalIteration_VE( litmax, littol, 0*noisy, Exxc, Eyyc, Exyc, Txxco, Tyyco, Txyco, Bc, mc, etaec );
        [ etav, Txxv, Tyyv, Txyv ] = M2Di2_LocalIteration_VE( litmax, littol, 0*noisy, Exxv, Eyyv, Exyv, Txxvo, Tyyvo, Txyvo, Bv, mv, etaev );
    end
    % Momentum residual
    Txxc     = 2*etac.*Exxc + etac./etaec.*Txxco;
    Tyyc     = 2*etac.*Eyyc + etac./etaec.*Tyyco;
    Txyv     = 2*etav.*Exyv + etav./etaev.*Txyvo;
    % Momentum residual
    Res_x      = diff(-Pt + Txxc,1,1)/dx + diff(Txyv(2:end-1,:),1,2)/dy;
    Res_y      = diff(-Pt + Tyyc,1,2)/dy + diff(Txyv(:,2:end-1),1,1)/dx;
    RMe        = [Res_x(:) ; Res_y(:)];
