...
for i=1:length(P_trial_vec)
    ...
    Ts_trial   = P_trial*sin_phis + cos_phis*Cs;
    ...
    if Tt_trial>Ts_trial % If shear
        ...
        P_corr = -(Cs.*cos_phi) ./ (-a+sin_phi);
        P_corr0  = P_corr
        T_corr = -a*P_corr;
        
        % solve for psi from T-corr
        sin_psi = eta_ve.*(Cs.*cos_phi + P_trial.*sin_phi - T_corr)./(K.*dt.*sin_phi.*(T_corr - T_trial));
        % solve for psi from P-corr - idem
        sin_psi = eta_ve.*(-P_corr + P_trial)./(K.*dt.*(Cs.*cos_phi + P_corr.*sin_phi - T_trial));
        
    end
    
    % Corrected
    f_trial = T_trial - P_trial*sin_phi - cos_phi*Cs;
    gdot    = f_trial / (eta_ve + K*dt*sin_phi*sin_psi);
    T_corr  = T_trial - eta_ve*gdot;
    P_corr  = P_trial + K*dt*gdot*sin_psi
    f_corr  = T_corr - P_corr*sin_phi - cos_phi*Cs
