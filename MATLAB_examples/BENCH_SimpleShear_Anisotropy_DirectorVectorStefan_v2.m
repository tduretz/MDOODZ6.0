...
% test_name = 'StressEvolConstantAnisotropy';
test_name = 'StressEvolFiniteStrainAnisotropy';
% Mdoodz    = load(test_name);
...
mu_N        = 1;
mu_S        = mu_N/10;
C_ISO       = 2*mu_N*[1 0 0; 0 1 0; 0 0 1/2];
w12         = 1/2*(dudy-dvdx);
e12         = 1/2*(dudy+dvdx);
L           = [dudx dudy; dvdx dvdy];
W           = [0 w12; -w12 0];
D           = [0 e12; e12 0];
D_vec       = [dudx dvdy 2*e12]';
Tau_vec     = C_ISO*D_vec;
Tauxy       = Tau_vec(3);
Gamma       = 0;
F_inc       = [1 dt*dudy; 0 1];
...
%     C_ANI           = [-a0 a0 -a1; a0 -a0 a1; -a1 a1 -1/2+a0];
    C_ANI           = [-a0 a0 a1; a0 -a0 -a1; a1 -a1 -1/2+a0];
    
    if strcmp(test_name, 'StressEvolFiniteStrainAnisotropy') == 1
        mu_S            = mu_N / Aratio; %min(100,Aratio);
    end
    ...
    if mod(it,20)==0
        subplot(221)
        hold on
        plot(Mdoodz.timevec/2.0, Mdoodz.txzvec, '-k')
        plot(Gamma,Tauxy,'-ok'), grid on
        legend('Mdoodz', 'Stefan script','location','northeast')
        xlabel('\gamma')
        ylabel('\tau_{xy}')
...
save('StefanFiniteStrainAnisotropyTest','Tauxy','Nx','Ny')

% print(test_name, '-dpng', '-r300')
