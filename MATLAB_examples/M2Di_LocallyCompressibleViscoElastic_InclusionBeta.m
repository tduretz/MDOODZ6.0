...
rad     = Ly/7.5;                                                            % Inclusion radius
B0      = 1/(1e-10*1e-5);                                                             % Background bulk modulus
B_i     = 0*1/(1e-5);                                                             % Inclusion bulk modulus
penalty = 1e-2;
G0      = 1e10;
dt      = 0.1*mus_i*B0;
VE      = 1;
...
Bc  ( sqrt(XC2        .^2 + YC2.^2) < rad) = B_i;
if locally_compressible==1
    comp_flag                         =     ones(nx  ,ny  );
    comp_flag( sqrt(XC2        .^2 + YC2.^2) < rad) = 0;
else
    comp_flag                         =    zeros(nx  ,ny  );
end
...
V      = (comp_flag(:)).*ones(nx*ny,1).*Bc(:)./dt + (1-comp_flag(:)).*ones(nx*ny,1)*penalty;
...
for istep=1:nt
    ...
    % BC on div
    BcD                 = (comp_flag(:)).*Pto(:).*Bc(:)./dt + (1-comp_flag(:)).*zeros(size(div,1),1);
    ...
    compD = diag(comp_flag(:));

    
    if solver == 0
        % This is the M2Di solver
        PPI      = spdiags(1./diag(PP),0,PP);
        Kt       = K - grad*(PPI*div);                                          % PPI*DivV = PP\DivV
        [Kcc,e,s] = chol(Kt,'lower','vector');
        for it=1:MaxIterL
            Rhs  = BcK  - grad*(PPI*BcD + p);                               % Powell-Hestenes
            u(s) = cs_ltsolve(Kcc,cs_lsolve(Kcc,Rhs(s))); 
            p    = p + PPI*(BcD - div*u);
            du   = BcK -   K*u -      grad*p;
            dp   = BcD - div*u;
        ...
        if norm(dp)/length(dp) < tolL, break, end
        end
    ...
    % Evaluate rheology from total stress (invalid for incompressible)
...
%     norm(Txx(:)+Tyy(:)+Tzz(:))
    ...
    % Evaluate rheology from stress deviators and pressure independently
    Txx = 2*etac.*(exx-1/3*divV);
    Tyy = 2*etac.*(eyy-1/3*divV);
    Tzz = 2*etac.*(0  -1/3*divV);
    norm(Txx(:)+Tyy(:)+Tzz(:))
    Sxx = -Pt + Txx; Syy = -Pt + Tyy; Szz = -Pt + Tzz;
    Pt1  = -1/3*(Sxx+Syy+Szz);
    ...
    % Check expression for eyyd
    eyyd1 = -exxd + 1/3*divV;
    norm(eyyd(:) - eyyd1(:))
    
    % Check expression for Tyy
    Tyy1 = -Txx + 2/3*etac.*divV;
    norm(Tyy(:) - Tyy1(:))
    ...
    subplot(222),imagesc(xc,yc,flipud(log10(abs(divV')))),axis image,colorbar, title('div V'), drawnow
